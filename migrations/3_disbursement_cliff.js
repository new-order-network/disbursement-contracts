...
module.exports = async (deployer) => {
  ...
  if (!receiver || !wallet || !disbursementPeriod || startDate == undefined) {
    throw Error(
      "Fill the deployment parameters in deploy-params.js. All are required."
    );
  }
  ...
  if (disbursementPeriod > fourYears) {
    throw Error(
      `disbursement period should be in seconds and no longer than 4 years. Got ${disbursementPeriod}`
    );
  }

  if (startDate < currentEpochInSeconds && startDate != 0) {
    throw Error(
      `startDate should be either 0 or higher than current time. Got ${startDate}`
    );
  }
  if (!isAddress(token)) {
  ...
  console.log(
    "Deploying Disbursement contract with the following configuration:"
  );
  console.log(`receiver : ${receiver}`);
  console.log(`wallet: ${wallet}`);
  console.log(`token:${token}`);
  console.log(
    `disbursementPeriod: ${disbursementPeriod} seconds | ${
      disbursementPeriod / oneYear
    } years`
  );
  console.log(
    `startDate of distribution: ${startDate} epoch | ${new Date(
      startDate
    ).toISOString()}`
  );
